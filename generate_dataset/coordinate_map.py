...
class CoordinateMap:
    ...
    def add(self, filename, start, stop, overlap=False, pattern=""):
        ...
        if overlap == False:
            if self.does_overlap(filename, start, stop):
                return (
                    False,
                    "Error, overlaps were found: {} {} {}".format(
                        filename, start, stop
                    ),
                )
    ...
    def add_extend(self, filename, start, stop, pattern=""):
        """adds a new coordinate to the coordinate map if overlaps with another, will extend to the larger size"""
        if self.debug:
        ...
        if len(overlaps) == 0:
            ...
            o = overlaps[0]
            self.add(
                filename, o["new_start"], o["new_stop"], pattern=pattern, overlap=True
            )
            # if filename == "./data/i2b2_notes/167-02.txt":
            # 	print("One overlap:")
            # 	print(filename,start,stop,pattern)
            ...
            o2 = overlaps[-1]
            self.add(
                filename, o2["new_start"], o1["new_stop"], pattern=pattern, overlap=True
            )
            # if filename == "./data/i2b2_notes/167-02.txt":
    ...
    def does_overlap(self, filename, start, stop):
        ...
        ranges = [
            list(range(key, self.map[filename][key] + 1)) for key in self.map[filename]
        ]
