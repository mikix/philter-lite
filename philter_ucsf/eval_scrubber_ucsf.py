...
def comparison(filename, file1path, file2path, allpositive_dict):
    ...
    phi_reduced_list = [
        word for sent in phi_reduced_words for word in sent if word not in punctuation
    ]
    ...
    annotation_list = [
        word for sent in annotation_words for word in sent if word not in punctuation
    ]
    ...
    for word_index, marker_and_word in enumerate(ndiff(phi_r_list, annot_list)):
        ...
        if annot_word_index > 5 and annot_word_index < len(annot_list):
            annot_context = " ".join(
                annot_list[annot_word_index - 5 : annot_word_index + 6]
            )
        ...
        if marker_and_word[0] == "+" and re.findall(r"\w+", marker_and_word[2:]) != []:
            ...
            file_context_dict["false_positives"].append(
                [marker_and_word[2:], annot_context]
            )
            plus_counter += 1
            # print(annot_word_index, marker_and_word, annot_context)
        # In the phi reduced list but not in the annotation list
        elif (
            marker_and_word[0] == "-" and re.findall(r"\w+", marker_and_word[2:]) != []
        ):
                    ...
                    if tag == "Age":
                        if fn.isdigit():
                            ...
                            if int(fn) >= 90:
                                ...
                                file_context_dict["false_negatives"].append(
                                    [fn, tag, context]
                                )
    ...
    output += (
        "False Positive: "
        + " ".join([item[0] for item in summary_dict["false_positive"]])
        + "\n"
    )
    output += "FP number: " + str(len(summary_dict["false_positive"])) + "\n"
    output += (
        "False Negative: "
        + " ".join([item[0] for item in summary_dict["false_negative"]])
        + "\n"
    )
    ...
    if true_positive == 0 and len(summary_dict["false_negative"]) == 0:
        ...
        output += (
            "Recall: {:.2%}".format(
                true_positive / (true_positive + len(summary_dict["false_negative"]))
            )
            + "\n"
        )
    # print(true_positive, len(summary_dict['false_positive']))
    if (true_positive == 0 and len(summary_dict["false_positive"]) == 0) or (
        true_positive < 0
    ):
        ...
        output += (
            "Precision: {:.2%}".format(
                true_positive / (true_positive + len(summary_dict["false_positive"]))
            )
            + "\n"
        )
...
def main():
    ...
    ap.add_argument(
        "-a", "--annotation", required=True, help="Path to the annotated file, *.txt."
    )
    ap.add_argument(
        "-o",
        "--output",
    ...
    allpositive_dict = json.loads(
        open("/data/muenzenk/nlm_scrubber/ucsf_detailed_anno_dict.json").read()
    )
    ...
    if os.path.isfile(file1path) != os.path.isfile(file2path):
        ...
        if os.path.isfile(file1path):
            ...
            if file1name != file2name:
                ...
                summary_dict, output, file_fn_dict, file_context_dict = comparison(
                    file1name, file1path, file2path, allpositive_dict
                )
            ...
            for i in phi_reduced_dict.keys():
                if i in annotation_dict.keys():
                    ...
                    summary_dict, output, file_fn_dict, file_context_dict = comparison(
                        i, phi_reduced_dict[i], annotation_dict[i], allpositive_dict
                    )
            ...
            print(
                "{:d} out of {:d} phi reduced notes have been compared.".format(
                    processed_count - len(miss_file), processed_count
                )
            )
            print(
                "{} files have not found corresponding annotation as below.".format(
                    len(miss_file)
                )
            )
            # print('\n'.join(miss_file)+'\n')
            if processed_count != 0:
                ...
                if TP_all == 0 and FP_all == 0:
                    ...
                    output += (
                        "Precision: {:.2%}".format(TP_all / (TP_all + FP_all)) + "\n"
                    )
                summary_text += output
        # else:
        ...
        if if_update:
            json.dump(
                summary_dict_all, open(foutpath + "/summary_dict.json", "w"), indent=4
            )
        ...
        with open(foutpath + "/fp_tags_context.txt", "w") as fp_file:
            ...
            for key in all_files_context_dict:
                ...
                for current_list in current_list_all:
                    fp_file.write(
                        current_list[0] + "|" + current_list[1] + "|" + file + "\n"
                    )
